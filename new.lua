...
local function ResolveFluent(Config)
	...
	local Fluent =
		...
		or rawget(Env, "FluentRenewed")
		or rawget(_G, "Fluent")
		or rawget(_G, "FluentRenewed")
		or GetSharedValue("Fluent")
		or GetSharedValue("FluentRenewed")

	local Options =
		(Config and Config.Options)
		or (Fluent and Fluent.Options)
		or rawget(Env, "Options")
		or rawget(_G, "Options")
...
function ToggleRunner:_HookFluentUnloadOnce()
	...
	for _, SignalName in ipairs({ "OnUnload", "PostUnload", "Unloaded", "_unloaded", "Destroyed" }) do
		local Signal = rawget(Fluent, SignalName)
		local Connection = SafeConnect(Signal, function()
			self:StopAll()
		end)
...
function ToggleRunner:AddToggleFunc(TabObj, Flag, Func, Props, Opts)
	if not TabObj or type(TabObj.AddToggle) ~= "function" then
		error("ToggleRunner:AddToggleFunc(tabObj, ...) expects a tab object with :AddToggle().")
	end

	if type(Func) == "table" and (type(Props) == "function" or type(Props) == "string") then
		local OldProps = Func
		Func = Props
		Props = OldProps
	end
	...
	if Props.Default == nil and Props.Value ~= nil then
		Props.Default = Props.Value
	end

	local Opt = TabObj:AddToggle(Flag, Props)
...
function ToggleRunner:BindExistingToggle(Flag, Func, Opts)
	local Opt = type(self.Options) == "table" and self.Options[Flag]

	if not Opt then
		error(("ToggleRunner:BindExistingToggle(%s, ...) could not find Options[%s]."):format(tostring(Flag), tostring(Flag)))
	end
